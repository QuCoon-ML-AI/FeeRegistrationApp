...
import com.pwb.feeRegistrationApp.service.*
...
import com.pwb.feeRegistrationApp.utility.constant.*
import com.pwb.feeRegistrationApp.utility.*
...
class RegistrationController(
	...
	@GetMapping("/read-by-registration-user-id/{registrationUserId}")
	fun readByRegistrationUserId(@RequestHeader("Authorization") authorization:String, @PathVariable registrationUserId:Int, @PathVariable stage: String):RegistrationReadListResponse{
		environment.userInfo = environment.jwtUtil.privilegeAuthorization<LoginResponse>(
			ModulePrivilege(this::class.java.name.split(".").last().removeSuffix("Controller").camelToSnakeCase().uppercase(), PrivilegeConstant.READ),
			authorization)
		return registrationService.readByRegistrationUserId(registrationUserId)
	}
