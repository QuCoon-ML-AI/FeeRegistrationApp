...
class FeeController(
	...
	@GetMapping("/read-by-fee-name/{feeName}")
	fun readByFeeName(@RequestHeader("Authorization") authorization:String, @PathVariable feeName:String, @PathVariable stage: String):FeeReadListResponse{
		environment.userInfo = environment.jwtUtil.privilegeAuthorization<LoginResponse>(
			ModulePrivilege(this::class.java.name.split(".").last().removeSuffix("Controller").camelToSnakeCase().uppercase(), PrivilegeConstant.READ),
			authorization)
		return feeService.readByFeeName(feeName)
	}
